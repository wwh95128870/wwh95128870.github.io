...
function drawLine(e) {
    ...
    x = ((getCursorPosition(e)[0]- this.offsetLeft) * 1000 / document.getElementById("canvas").clientWidth) - this.offsetLeft/1000;
    y = ((getCursorPosition(e)[1]- this.offsetTop) * 1000 / document.getElementById("canvas").clientHeight) - this.offsetTop/1000;
    x = x.toFixed(0);
    y = y.toFixed(0);
...
function levelChangeController() {
    var gameWin = score == maxScore
    if (gameWin) {
        currentLevel++;
        setTimeout(function () {
            alert("you win, go to next level");
            init(ctx, gameLevel[currentLevel].blockArray, gameLevel[currentLevel].pin, gameLevel[currentLevel].maxScore);
